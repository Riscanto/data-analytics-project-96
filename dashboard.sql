...
leads_tab AS (
    ...
    INNER JOIN sessions AS s
        ON
            last_paid.visitor_id = s.visitor_id
            AND last_paid.max_paid_date = s.visit_date
    LEFT JOIN leads
        ON
            last_paid.visitor_id = leads.visitor_id
            AND last_paid.max_paid_date <= leads.created_at
    ...
    SELECT
        ...
        (
            CASE
                WHEN
                    visitors_count != 0
                    THEN (leads_count / visitors_count) * 100
            END
        ) AS visit_to_lead,
        (
            CASE
                WHEN leads_count != 0 THEN (purchases_count / leads_count) * 100
            END
        ) AS lead_to_purchase,
        (
            CASE
                WHEN visitors_count != 0 THEN (total_cost_sum / visitors_count)
            END
        ) AS cpu,
        (
        ...
        (
            CASE
                WHEN
                    purchases_count != 0
                    THEN (total_cost_sum / purchases_count)
            END
        ) AS cppu,
        (
            CASE
...
ct AS (
    ...
    WHERE rn = (SELECT (max(rn) * 90 / 100) AS p FROM leads_structure)
),
organic_visits AS (
    SELECT
...
corr_data AS (
    SELECT
        ...
        sum(cost) AS cost
    FROM organic_visits
    FULL JOIN ads_tab ON organic_visits.visit_date = ads_tab.campaign_date
    GROUP BY 1, 2
    ORDER BY 1
),
cr AS (
